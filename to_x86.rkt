  ...
  (lambda (alist)
    (lambda (e)
      (letrec ([lookup (lambda (table key)
                         (cond [(null? table) (error "key is not defined in table" table key)]
        ...
        [(? symbol?) (lookup alist e)]
          ...
          [`(let ([,x ,e]) ,body) 
           (let* ([new-x (gensym x)]
                  [new-alist (cons `(,x . ,new-x) alist)]
                  [new-e ((uniquify new-alist) e)])
             `(let ([,new-x ,new-e]) ,((uniquify new-alist) body)))]
...
(interp-tests "uniquify" (lambda (t) #t) '() uniquify "s1" '(1 2 3))
